...
func simulateRelays(t *testing.T,k Keeper, ctx *sdk.Ctx, maxRelays int) (npk crypto.PublicKey, memInvoices *types.Invoices, validHeader types.SessionHeader, keys simulateRelayKeys, storedInvoice types.StoredInvoice) {
	npk = getRandomPubKey()
	memInvoices = types.GetAllInvoices()
	...
	for j := 0; j < maxRelays; j++ {
		...
		memInvoices.AddToInvoice(validHeader, proof)
	}
	mockCtx := new(Ctx)
	...
	k.SetInvoices(mockCtx, []types.StoredInvoice{storedInvoice})
	keys = simulateRelayKeys{privateKey, clientKey}
	return
}
func createProof(private, client crypto.Ed25519PrivateKey, npk crypto.PublicKey, chain string, entropy int) types.RelayProof{
	aat := types.AAT{
		Version:              "0.0.1",
