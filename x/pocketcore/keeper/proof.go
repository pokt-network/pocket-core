...
func (k Keeper) SendProofTx(ctx sdk.Ctx, n client.Client, node *pc.PocketNode, proofTx func(cliCtx util.CLIContext, txBuilder auth.TxBuilder, merkleProof pc.MerkleProof, leafNode pc.Proof, evidenceType pc.EvidenceType) (*sdk.TxResponse, error)) {
	...
	for _, claim := range claims {
		...
		// Causing a mismatch on which merkle proof is generated and what is seen on chain via a claim.
		// In order to allow for best-effort process to submit a proof for the claim the servicer submitted
		// 1. Generate a merkle proof for claim's total proof if the claim total proofs doesn't exceed max relays per session
