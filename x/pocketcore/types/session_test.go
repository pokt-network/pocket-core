...
func TestNewSessionNodes(t *testing.T) {
	...
	sessionNodes, err := NewSessionNodes(
		newContext(t, false).WithAppVersion("0.0.0"),
		newContext(t, false).WithAppVersion("0.0.0"),
		k,
		ethereum,
		fakeSessionKey,
	...
	assert.True(t, sessionNodes.Contains(node12.Address))
	assert.True(t, sessionNodes.Contains(node11.Address))
	assert.True(t, sessionNodes.Contains(node6.Address))
	assert.True(t, sessionNodes.Contains(node2.Address))
	assert.True(t, sessionNodes.Contains(node4.Address))
	assert.Nil(t, sessionNodes.Validate(5))
	assert.NotNil(t, SessionNodes(make([]sdk.Address, 5)).Validate(5))
}

func TestNewSessionNodesAfterMaxChains(t *testing.T) {
	maxChainsUpgradeHeight := int64(10)

	codec.UpgradeFeatureMap[codec.EnforceMaxChainsUpdateKey] = maxChainsUpgradeHeight
	...
	assert.Len(t, sessionNodes, 5)
	assert.True(t, sessionNodes.Contains(node12.Address))
	assert.True(t, sessionNodes.Contains(node9.Address))
	assert.True(t, sessionNodes.Contains(node6.Address))
	assert.True(t, sessionNodes.Contains(node2.Address))
	assert.True(t, sessionNodes.Contains(node1.Address))
	assert.Nil(t, sessionNodes.Validate(5))
	assert.NotNil(t, SessionNodes(make([]sdk.Address, 5)).Validate(5))

	ctx = ctx.WithBlockHeight(maxChainsUpgradeHeight)
	...
	assert.Nil(t, err)
	assert.Len(t, sessionNodes, 5)
	assert.False(t, sessionNodes.Contains(node12.Address))
	assert.True(t, sessionNodes.Contains(node3.Address))
	assert.True(t, sessionNodes.Contains(node9.Address))
	assert.True(t, sessionNodes.Contains(node6.Address))
	assert.True(t, sessionNodes.Contains(node2.Address))
	assert.True(t, sessionNodes.Contains(node1.Address))
