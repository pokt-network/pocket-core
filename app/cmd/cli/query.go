...
var queryBlock = &cobra.Command{
	...
	Run: func(cmd *cobra.Command, args []string) {
		...
		if len(args) == 0 {
			...
			json.Unmarshal([]byte(res), &currentHeight)
			height = currentHeight.Height
		} else {
			var err error
			height, err = strconv.ParseInt(args[0],10,64)
			if err != nil {
				fmt.Println(" Invalid Height:",args[0],"\n",err)
				return
			}
		}
...
var queryBalance = &cobra.Command{
	...
	Run: func(cmd *cobra.Command, args []string) {
		...
		if len(args) == 1 {
			...
			if err != nil {
				fmt.Println(" Invalid Height:",args[1],"\n",err)
				return
			}
		}
...
var queryNodeClaim = &cobra.Command{
	...
	Run: func(cmd *cobra.Command, args []string) {
		...
		if len(args) == 5 {
			...
			if err != nil {
				fmt.Println(" Invalid Height:",args[5],"\n",err)
				return
			}
		}
		sessionheight, err := strconv.Atoi(args[4])
		if err != nil {
			fmt.Println(" Invalid Session Height:",args[4],"\n",err)
			return
		}
